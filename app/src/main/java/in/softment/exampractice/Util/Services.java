package in.softment.exampractice.Util;
...
import in.softment.exampractice.EnterInformationActivity;
...
import in.softment.exampractice.MainActivity;
...
public class Services {
    ...
    public static List<String> getRandomArray(int totalCategoryQuestion, int totalEachSetQuestion){
        ArrayList<String> totalQuestionsArray = new ArrayList<>();
       for (int i = 1; i <= totalCategoryQuestion; i++) {
           totalQuestionsArray.add(String.valueOf(i));
       }

        Collections.shuffle(totalQuestionsArray);

       return totalQuestionsArray.subList(0, totalEachSetQuestion);
                ...
                if (task.isSuccessful()) {
                    ...
                    if (documentSnapshot != null && documentSnapshot.exists()) {
                        ...
                        if (UserModel.data != null) {
                            ...
                            Intent intent;
                            if (UserModel.data.fullName.isEmpty()) {
                                intent = new Intent(context, EnterInformationActivity.class);
    ...
    public static void getAllQuestion(String catId,List<String> myList, QuestionsListener questionsListener){



        ArrayList<QuestionModel> questionModels =  new ArrayList<>();
        FirebaseFirestore.getInstance().collection("Categories").document(catId).collection("Questions").whereIn("questionId", myList).get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
            @Override
            public void onComplete(@NonNull Task<QuerySnapshot> task) {
                if (task.isSuccessful()) {
                    ...
                    questionsListener.onCallback(questionModels, task.getException().getLocalizedMessage());
                }
            }
        });
    ...
    public static void goMembership(Context context, View view){
        BottomSheetDialog membershipDialog = new BottomSheetDialog(context, R.style.BottomSheetStyle);
        View view2 = LayoutInflater.from(context).inflate(R.layout.membership_bottom_layout,(LinearLayout)view.findViewById(R.id.membershipSheet));
        CheckBox monthly_checkbox = view2.findViewById(R.id.monthly_check);
        CheckBox yearly_checkbox = view2.findViewById(R.id.yearly_check);
        TextView monthly_priceTV =  view2.findViewById(R.id.normal_price);
        TextView yearly_priceTV = view2.findViewById(R.id.yearly_price);
        TextView freeTag = view2.findViewById(R.id.freeTag);
        TextView freeTag1 = view2.findViewById(R.id.freeTag1);


        //TermsofUse
        view2.findViewById(R.id.termsOfUse).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent browserIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("https://softment.in/darwinning/termsofuse.html"));
        ...
        view2.findViewById(R.id.privacyPolicy).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent browserIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("https://softment.in/darwinning/privacy.html"));
                context.startActivity(browserIntent);
            }
        });
    ...
    public static void getAllCategory(CategoriesListener categoriesListener){
        ArrayList<CategoryModel> categoryModels =  new ArrayList<>();
        Query query  = FirebaseFirestore.getInstance().collection("Categories").orderBy("name").whereEqualTo("available",true);
    ...
    public static void getSolvedPaper(String userId, String subCatId, SolvedPaperListener solvedPaperListener){


        FirebaseFirestore.getInstance().collection("Users").document(userId).collection("SolvedPapers").document(subCatId)
                    ...
                    public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                        if (task.isSuccessful()) {
                            ...
                            if (task.getResult() != null && task.getResult().exists()) {
                                SolvedPaperModel solvedPaperModel = task.getResult().toObject(SolvedPaperModel.class);
                                solvedPaperListener.onCallBack(solvedPaperModel,null);
                            }
                            else {
                                solvedPaperListener.onCallBack(null,"NOT_FOUND");
                            }


                        }
                        else {
                           solvedPaperListener.onCallBack(null,task.getException().getLocalizedMessage());
                        }
                    }
